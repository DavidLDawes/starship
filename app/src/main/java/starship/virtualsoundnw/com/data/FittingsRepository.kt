...
interface FittingsRepository {
    fun getFittingForShip(shipId: Int): Flow<Fitting?>
    suspend fun updateFitting(fitting: Fitting)
    suspend fun updateSensorType(shipId: Int, sensorType: SensorType)
    suspend fun updateComputerModel(shipId: Int, computerModel: ComputerModel)
    suspend fun clearFittingForShip(shipId: Int)
}